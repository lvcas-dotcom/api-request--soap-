...
class CadastroService:
    ...
    def extrair_todos_cadastros(
        self, codigo_inicio=0, codigo_fim=200000, intervalo_size=100
    ):
        """
        ...
        try:
            ...
            for i, (inicio, fim) in enumerate(intervalos):
                ...
                if (
                    len(todos_cadastros) > 0
                    and len(todos_cadastros) % self.app_config.save_interval == 0
                ):
                    self.file_storage_service.salvar_progresso_parcial(
                        todos_cadastros, sufixo="auto_backup"
                    )
            ...
            CLIInterface.mostrar_conclusao(
                len(todos_cadastros), tracker.requisicoes_realizadas, tempo_execucao
            )
            ...
            if todos_cadastros:
                ...
                stats = self.statistics_service.gerar_estatisticas_completas(
                    todos_cadastros
                )
                codigos_info = self.statistics_service.analisar_codigos_cadastro(
                    todos_cadastros
                )
    ...
    def _buscar_por_intervalo(self, codigo_inicio, codigo_fim):
        ...
        try:
            ...
            cadastros_raw = self.soap_client.buscar_cadastro_geral(
                codigo_cadastro=codigo_intervalo, tipo_consulta=1, situacao=1
            )
            ...
            CLIInterface.mostrar_erro(
                f"Erro inesperado no intervalo {codigo_intervalo}: {e}"
            )
            return []
        ...
        try:
            ...
            for campo in [
                ...
                if campo in cadastro_raw and cadastro_raw[campo]:
                    try:
                        cadastro_raw[campo] = float(
                            str(cadastro_raw[campo]).replace(",", ".")
                        )
    ...
    def buscar_cadastro_completo(
        self, codigo_cadastro: str
    ) -> Optional[CadastroImobiliario]:
        """
        ...
        try:
            ...
            proprietarios = self.soap_client.buscar_proprietarios(
                codigo_cadastro=codigo_cadastro
            )
